...
TransposeConv shape out:
(INshape-1)*S -2p+k+outpadding = OUTshape
*/
#ifndef MNET_TRANSPOSE_CONV2D_H_INCLUDED
#define MNET_TRANSPOSE_CONV2D_H_INCLUDED
	...
	)
{
	for(int cout_n=0;cout_n<CONFIG_C::CHout;cout_n++)
	{
		...
		for(int ll=0;ll<CONFIG_C::Hout*CONFIG_C::Wout;ll++){
			feature_out[cout_n*CONFIG_C::Hout*CONFIG_C::Wout+ll]+=bias[cout_n];
...
*/
template <typename CONFIG_C>
void ConvTranspose2d_pp_notfinish(
		typename CONFIG_C::Dtype_f feature_in[],
		typename CONFIG_C::Dtype_w W[],
		typename CONFIG_C::Dtype_b bias[],
		typename CONFIG_C::Dtype_f feature_out[]
...
{
	//need to process the input FIRSTLY!!!!
	int H_pp=(CONFIG_C::Hin-1)*(CONFIG_C::Sh)+1;
	int W_pp=(CONFIG_C::Win-1)*(CONFIG_C::Sw)+1;
	typename CONFIG_C::Dtype_f feature_in_pp[CONFIG_C::CHin*(H_pp)*(W_pp)];
	for(int i=0;i<CONFIG_C::CHin*(H_pp)*(W_pp);i++){feature_in_pp[i]=0;}

	//padding the feature_in, put (S-1) zeros between two elements. Do not consider the padding, normal conv will take over it.
	for(int i=0;i<CONFIG_C::CHin;i++){
		for(int j=0;j<CONFIG_C::Hin;j++){
			for(int k=0;k<CONFIG_C::Win;k++){
				//cin=i, h=j*Sh, w=k*Sw
				feature_in_pp[i*H_pp*W_pp+j*CONFIG_C::Sh*W_pp+k*CONFIG_C::Sw]
					=feature_in[i*CONFIG_C::Hin*CONFIG_C::Win+j*CONFIG_C::Win+k];
			}
		}
	}

	//actually we should do vertical and hirzontal shift for W.
	//but we can achieve it by index changing.


	//do normal conv2d, in stride=1
	for(int cout_n=0;cout_n<CONFIG_C::CHout;cout_n++)
	{
			...
			for(int j=0;j<CONFIG_C::Wout;j++)
			{
				...
				for(int ii=0;ii<CONFIG_C::Kh;ii++)
				{
					for(int jj=0;jj<CONFIG_C::Kw;jj++)
					{
						typename CONFIG_C::Index_t h=i*CONFIG_C::Sh-CONFIG_C::pad_h+ii;
						typename CONFIG_C::Index_t w=j*CONFIG_C::Sw-CONFIG_C::pad_w+jj;
						if(h>=0 && w>=0 && h<H_pp && w<W_pp)
						{
							for(int cin_n=0;cin_n<CONFIG_C::CHin;cin_n++)
							{
								//Feature in shape: (CHin,Hin,Win) == [CHin][Hin][Win]
								//Kernel shape: (CHin, CHout, kH, kW) == [CHin][CHout][Kh][Kw]
								//Feature out shape:(CHout,Hout,Wout​) == [CHout][Hout][Wout]
								//Feature_out[CHout][Hout][Wout] = Feature_in[CHin][Hin][Win] * W[CHout][CHin][Kh][Kw] + bias[CHout]
								typename CONFIG_C::Dtype_tmp tp=
								feature_in_pp[cin_n*H_pp*W_pp+h*W_pp+w]*
								W[cin_n*CONFIG_C::CHout*CONFIG_C::Kh*CONFIG_C::Kw
									+cout_n*CONFIG_C::Kh*CONFIG_C::Kw
									+(CONFIG_C::Kh-1-ii)*CONFIG_C::Kw
									+(CONFIG_C::Kw-1-jj)
									// +ii*CONFIG_C::Kw
									// +jj
									];
